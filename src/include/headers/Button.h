...
public:
    Button(int x, int y, int w, int h, SDL_Color base, SDL_Color highLight, SDL_Color bastTextColor, SDL_Color highLightText, const char* buttonText, int fontSize) 
        : Rect{x - w/2, y - h/2, w, h}, baseColor(base), highLightColor(highLight), baseTextColor(baseTextColor), textHightLightColor(highLightText), text(buttonText), _fontSize(fontSize) {
